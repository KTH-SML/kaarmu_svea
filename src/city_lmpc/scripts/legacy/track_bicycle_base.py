...
class TrackBicycleBase(Node):
    ...
    def __init__(self):
        ...
        # Track Parameters
        self.look_ahead_base = 0.32
        self.look_ahead_factor = 0.0
        self.track = Track(arcs_bendy, x_s0=self.x_s0,
                           y_s0=self.y_s0, phi_s0=self.phi_s0, flip=self.FLIPPED_REF)
    ...
    def get_trajectory(self, x):
        ...
        for j in range(N + 1):
            ...
            if curv > 0 or curv < 0:
                ...
                x_ref[2, j] = self.v_ref

        if self.FLIPPED_REF:
            x_ref[-1, :] = self.track.points_array[4, i:i + N + 1] - pi
        else:
