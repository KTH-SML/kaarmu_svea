...
BUILD_CONTEXT="https://github.com/kth-sml/city_lmpc_ws.git"
IMAGE_TAG="$(basename "$BUILD_CONTEXT")"
IMAGE_TAG="${IMAGE_TAG%%.*}"
CONTAINER_NAME="$REPOSITORY_NAME"

# Trick to allow docker to cache builds (if you have the workspace locally)
[ -d "../$IMAGE_TAG" ] && BUILD_CONTEXT="$(realpath "../$IMAGE_TAG")"
