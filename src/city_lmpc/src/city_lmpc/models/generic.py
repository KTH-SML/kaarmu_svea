...
import numpy as np
from casadi import integrator, vertcat, horzcat, DM, DaeBuilder, Function
...
class GenericModel():
    ...
    def _build_simulator(self):
        ...
        self._F = Function("F", [x, u], [x_next], ["x", "u"], ["x_next"])
    ...
    def f(self, x, u, state_noise=False, input_noise=False) -> Tuple[float, float]:
        if input_noise:
            n_inputs = len(self.dae.u)
            u[:n_inputs,
                0] += np.random.normal(0, self.INPUT_NOISE_STD, n_inputs)  # apply noise to inputs only not parameters
    ...
    def sim(self, time_steps, u, x0=None, state_noise=False, input_noise=False, is_logging=True):
        assert time_steps == u.shape[1], "Input second dimension {} must match time_steps {}".format(
            u.shape[1], time_steps)
        if type(x0) == type(None):
            x0 = self.state_trajectory[-1]
        n_states = len(self.dae.x)
        res = DM.zeros((n_states, time_steps))
        x_next = x0
        for t in range(time_steps):
            ...
            x_next, u_applied = self.f(
                ...
                x_next, u[:, [t]], state_noise, input_noise)
            # brackets on t to make the selection a column vector (n_states,1) instead of (n_states,)
            res[:, [t]] = x_next
            if is_logging:
