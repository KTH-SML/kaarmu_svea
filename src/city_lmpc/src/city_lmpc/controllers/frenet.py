...
class FrenetMPC():
    ...
    def _build_optimizer(self):
        ...
        self.x0 = x0
        curvature = opti.parameter(1, self.N)
        self.curvature = curvature
        s_0_arc = opti.parameter(1, self.N)
        self.s_0_arc = s_0_arc
        phi_0_arc = opti.parameter(1, self.N)
        self.phi_0_arc = phi_0_arc
        ...
        for t in range(N):
            x_next, _ = self.model.f(
                ...
                x[:, t], vertcat(u[:, t], curvature[:, t], s_0_arc[:, t], phi_0_arc[:, t]), state_noise=self.state_noise, input_noise=self.input_noise)
            opti.subject_to(
                x[:, t + 1] == x_next)
