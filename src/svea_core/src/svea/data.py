...
class BasicDataHandler(object):
    ...
    SAVE_DIR = os.path.join(dirname, '../../resources/logs/')
    ...
    def __init__(self, vehicle_name=''):
        ...
        # best not to include ROS objects in this list
        self.vars_to_save = ["x", "y", "yaw", "v", "t",
                             "ctrl_steer", "ctrl_v", "ctrl_trans", "ctrl_t",
                             "emergency_start_end", "emergency_reasons",
                             "emergency_t",
                             "start_time", "vehicle_name"]
    ...
    def log_ctrl(self, steering, velocity, transmission, t):
        ...
        :type velocity: float
        :param transmission: Transmission input
        :type transmission: int
        :param t: Timestamp of control input [s]
        :type t: float
        """
        self.ctrl_steer.append(steering)
        self.ctrl_v.append(velocity)
        self.ctrl_trans.append(transmission)
    ...
    def save_data(self, filename):
        ...
        :param filename: desired file path with logs/ as root
        :type filename: str
        """
        data_dict = deepcopy(self.__dict__)
        for key in data_dict.keys():
            if not key in self.vars_to_save:
                data_dict.pop(key)
        file_handler = open(self.SAVE_DIR+filename+".pkl", 'w')
...
class TrajDataHandler(BasicDataHandler):
    ...
    def __init__(self, vehicle_name='', traj_x=[], traj_y=[]):
        ...
        self.traj_y = traj_y
        self.target = None
        self.vars_to_save += ["traj_x", "traj_y", "target"]
