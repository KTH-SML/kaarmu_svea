...
"""
Module containing teleoperation interfaces for the RC remote, etc.
"""
...
from logitech_wheel_msgs.msg import LogitechWheelCmd
...
class LogitechInterface(object):
    """Interface handling the reception of teleop inputs generated from
    SML's Logitech steering wheel ROS driver.
    ...
    #TODO: complete feature set for this teleop inteface
    ...
    def __init__(self, vehicle_name=""):
        self.vehicle_name = vehicle_name
        self._teleop_topic = '/logitech_drive_cmd'
        self.teleop_cmd = LogitechWheelCmd()
    ...
    def _init_and_spin_ros(self):
        rospy.loginfo('Starting Logitech Interface Node for '
                      + self.vehicle_name)
        self.node_name = 'control_interface'
        self._start_listen()
        rospy.sleep(0.1)
        rospy.loginfo("{} Logitech Interface successfully initialized"
                      .format(self.vehicle_name))
        rospy.spin()

    def _start_listen(self):
        rospy.Subscriber(self._teleop_topic,
                         LogitechWheelCmd,
    ...
    def operation_mode(self):
        return self.teleop_cmd.operation_mode # string, ["auto", "takeover"]


class SafeTeleopInterface(object):
    """Interface handling the reception of teleop inputs generated from
    a SafeTeleopManager.
    ...
    def _init_and_spin_ros(self):
        rospy.loginfo('Starting Safe Teleop Interface Node for '
                      + self.vehicle_name)
        self._start_listen()
        rospy.spin()

    def _start_listen(self):
        rospy.Subscriber(self._input_topic,
                         DriveInput,
                         self._read_input)

    def _read_input(self, msg):
        self.drive_input = msg
